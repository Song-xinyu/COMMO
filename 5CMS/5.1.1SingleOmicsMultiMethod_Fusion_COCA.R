SOMMFusionPack_COCAv2=function(ResultSingleMethod, Data, ClustMethod, OmicsLevel){
  ...
  {
    COCA_result_list=ConsensusClusterPlus(t(sparse_cross_heatmap), maxK = 3, reps = 100, pItem = 0.9, clusterAlg = "hc", distance="pearson", seed = 8715)
    cluster_number=2
    cluster_result=unname(COCA_result_list[[cluster_number]]$consensusClass)
  }else{
    COCA_result_list=ConsensusClusterPlus(t(sparse_cross_heatmap), maxK = 8, reps = 100, pItem = 0.9, clusterAlg = "hc", distance="pearson", seed = 8715)
    cluster_number=OptClustNum_Fori(ConsensusClusterPlus_result=COCA_result_list)
    cluster_result=unname(COCA_result_list[[cluster_number]]$consensusClass)
  }
  rm(COCA_result_list)
